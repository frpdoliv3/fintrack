...
internal interface IAPIErrorParserService
{
    public IError ParseErrors(ModelStateDictionary modelState);
}

internal class APIErrorParserService: IAPIErrorParserService
{
    ...
    public IError ParseErrors(ModelStateDictionary actionModelState)
    {
        IError root = new ObjectError();
        foreach (var modelState in actionModelState)
        {
